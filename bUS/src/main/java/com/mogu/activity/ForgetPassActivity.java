...
public class ForgetPassActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_m_forget_pass);
		initView();
	...
	private void initView() {
		TitleUtils.setTitle(ForgetPassActivity.this, R.id.topbar, "忘记密码");
		...
		if (isMobileNum(phone)) {
			...
			x.http().post(params, new CommonCallback<String>() {
				...
				public void onCancelled(CancelledException arg0) {
					...
					Toast.makeText(ForgetPassActivity.this, "嗷嗷",
				...
				public void onFinished() {
					// TODO Auto-generated method stub
					Toast.makeText(ForgetPassActivity.this, "完成",
							Toast.LENGTH_SHORT).show();
				}
				...
				public void onSuccess(String arg0) {
					...
					try {
						...
						if (obj.get("code").equals("1")) {
							Toast.makeText(ForgetPassActivity.this, "验证码发送成功",
									Toast.LENGTH_SHORT).show();
						} 
			...
			Toast.makeText(ForgetPassActivity.this, "输入正确格式手机号",
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void resetPassword() {
		...
		Log.d("pass", password);
		if (!isMobileNum(phone)) {
			Toast.makeText(ForgetPassActivity.this, "手机号格式不正确",
					Toast.LENGTH_SHORT).show();
		} else if (password.contains(" ")) {
			Toast.makeText(ForgetPassActivity.this, "密码不能包含空格",
					Toast.LENGTH_SHORT).show();
		} else if (password.length() < 6 || password.length() > 15) {
			Toast.makeText(ForgetPassActivity.this, "密码为6-15位",
					Toast.LENGTH_SHORT).show();
		} else if (yanzhengma.length() < 6 || yanzhengma.length() > 6) {
			Toast.makeText(ForgetPassActivity.this, "请输入6位验证码",
					Toast.LENGTH_SHORT).show();
		} else {
				...
				public void onSuccess(String arg0) {
					try {
						...
						if (obj.get("code").equals("1")) {
							...
							Toast.makeText(ForgetPassActivity.this, "请检查验证码",
									Toast.LENGTH_SHORT).show();
						}
					} catch (JSONException e) {
						// TODO Auto-generated catch block
	...
	private void MyRegist(String phone, String pass) {
		RequestParams params = new RequestParams(Url.RESET_PASSWORD);
		params.addBodyParameter("sjhm00", phone);
		...
		x.http().get(params, new CommonCallback<String>() {
			...
			public void onSuccess(String arg0) {
				try {
					JSONObject obj = new JSONObject(arg0);
					if (obj.get("code").equals("1")) {
						Toast.makeText(ForgetPassActivity.this, "您已重置密码",
								Toast.LENGTH_SHORT).show();
						finish();
					}else {
						Toast.makeText(ForgetPassActivity.this, obj.getString("msg"),
								Toast.LENGTH_SHORT).show();
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
