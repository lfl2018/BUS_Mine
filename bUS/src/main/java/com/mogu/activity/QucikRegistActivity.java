 ...
 * @date 2015年8月28日
 */
public class QucikRegistActivity extends BaseActivity implements OnClickListener {
	// 输入手机号码
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_ihealth_register);
		initView();
		...
		agree.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
				if (arg1 != true) {
					...
					btnRegister.setClickable(arg1);
					btnRegister.setBackgroundResource(drawable.selector_btn);
				}
	...
	private void initView() {
		...
		btnBack = (TextView) findViewById(R.id.btn_return);
	...
	public static void actionIntent(Context context) {
		Intent intent = new Intent(context, QucikRegistActivity.class);
		...
		if (isMobileNum(phone)) {
			...
			x.http().post(params, new CommonCallback<String>() {
				...
				public void onCancelled(CancelledException arg0) {
					...
					Toast.makeText(QucikRegistActivity.this, "嗷嗷",
				...
				public void onFinished() {
					// TODO Auto-generated method stub
					Toast.makeText(QucikRegistActivity.this, "完成",
							Toast.LENGTH_SHORT).show();
				}
				...
				public void onSuccess(String arg0) {
					...
					try {
						...
						if (obj.get("code").equals("1")) {
							Toast.makeText(QucikRegistActivity.this, "验证码发送成功",
									Toast.LENGTH_SHORT).show();
						} 
			...
			Toast.makeText(QucikRegistActivity.this, "输入正确格式手机号",
					Toast.LENGTH_SHORT).show();
		}
	}
	...
	private void rigister() {
		...
		Log.d("pass", password);
		if (!isMobileNum(phone)) {
			Toast.makeText(QucikRegistActivity.this, "手机号格式不正确",
					Toast.LENGTH_SHORT).show();
		} else if (password.contains(" ")) {
			Toast.makeText(QucikRegistActivity.this, "密码不能包含空格",
					Toast.LENGTH_SHORT).show();
		} else if (password.length() < 6 || password.length() > 15) {
			Toast.makeText(QucikRegistActivity.this, "密码为6-15位",
					Toast.LENGTH_SHORT).show();
		} else if (yanzhengma.length() < 6 || yanzhengma.length() > 6) {
			Toast.makeText(QucikRegistActivity.this, "请输入6位验证码",
					Toast.LENGTH_SHORT).show();
		} else {
				...
				public void onSuccess(String arg0) {
					...
					try {
						...
						if (obj.get("code").equals("1")) {
							...
							Toast.makeText(QucikRegistActivity.this, "请检查验证码",
									Toast.LENGTH_SHORT).show();
						}
					} catch (JSONException e) {
						// TODO Auto-generated catch block
		...
		x.http().get(params, new CommonCallback<String>() {
			...
			public void onSuccess(String arg0) {
				try {
					JSONObject obj = new JSONObject(arg0);
					if (obj.get("code").equals("1")) {
						Toast.makeText(QucikRegistActivity.this, "恭喜您注册成功",
								Toast.LENGTH_SHORT).show();
						finish();
					}else {
						Toast.makeText(QucikRegistActivity.this, obj.getString("msg"),
								Toast.LENGTH_SHORT).show();
					}
				} catch (JSONException e) {
					// TODO Auto-generated catch block
